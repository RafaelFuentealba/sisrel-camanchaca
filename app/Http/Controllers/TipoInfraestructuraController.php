...
class TipoInfraestructuraController extends Controller {
    ...
    public function store(Request $request) {
        ...
        [
            ...
            'valorizacion.min' => 'Valorización de la infraestructura debe ser un número mayor o igual que cero.'
        ]);
    ...
    public function update(Request $request, $tiin_codigo) {
        ...
        if (!$tiinVerificar) return redirect()->back()->with('errorInfraestructura', 'Infraestructura no se encuentra registrada.');
    ...
    public function destroy($tiin_codigo) {
        $tiinVerificar = DB::table('costos_infraestructura')
            ->select('inic_codigo')
            ...
            ->get();
        if (sizeof($tiinVerificar) > 0) return redirect()->back()->with('errorInfraestructura', 'No se puede eliminar la infraestructura porque posee iniciativas asociadas.');
